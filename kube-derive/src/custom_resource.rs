...
struct ValidateSchemars {
    schema_with: String,
}

#[derive(FromDeriveInput)]
#[darling(attributes(validated), supports(struct_named))]
struct CELSettings {
    ident: Ident,
    mod_name: Option<Path>,
    #[darling(default)]
    crates: Crates,
}
            ...
            if manual {
                ...
                match ValidateSchemars::from_attributes(&field.attrs) {
                    Ok(schemars_attr) if schemars_attr.schema_with != validator => return syn::Error::new(
                        attr.path().span(),
                        format!(
                            r#"#[validated(..)] rule must declare #[schemars(schema_with = "{validator}")]"#
                        ),
                    )
                    .to_compile_error(),
                    Err(e) => return e.with_span(&attr.meta).write_errors(),
